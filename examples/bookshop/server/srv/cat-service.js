...
class CatalogService extends cds.ApplicationService {
  init() {
    ...
    this.on("getBestBook", async () => {
      const book = await cds.run(SELECT.one.from(Books).where("ID", "=", 201));
      return book;
    });

    this.on("getBestBooks", async () => {
      return await cds.run(SELECT.from(Books).limit(5));
    });

    this.on("getONEStockPrice", async () => {
      const book = await cds.run(SELECT.one.from(Books).where("ID", "=", 201));
      return {
        price: book.price,
        stock: book.stock,
    ...
    this.on("GET_STOCK_PRICES", async () => {
      const books = await cds.run(SELECT.from(Books).limit(5));
      return books.map((b) => ({
        price: b.price,
        stock: b.stock,
      }));
