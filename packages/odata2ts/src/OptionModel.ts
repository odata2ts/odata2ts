...
/**
 * In which way a property is managed by the server, which decides where it shows up: a property the
 * client may not write is absent from the editable models, one it may not read is absent from the
 * model itself.
 *
 * The state is derived from the `Org.OData.Core.V1` annotations of the service, or configured by hand
 * via `managed` in the property options - configuration always wins.
 */
export enum ManagedState {
  /**
   * Not managed at all, whatever the service or the key detection would have to say about it. This is
   * the state to configure in order to overrule everything else; not stating anything is different, as
   * it leaves the decision to annotations and key detection.
   */
  off = "off",
  /**
   * The client may only read this property: `Core.Computed` (the server generates the value on insert
   * and update) or `Core.Permissions` allowing nothing but `Read`.
   */
  readOnly = "readOnly",
  /**
   * The client may only write this property: `Core.Permissions` allowing nothing but `Write`. Such a
   * property never travels in a response, so it is absent from the model and only known to the
   * editable one.
   */
  writeOnly = "writeOnly",
  /**
   * `Core.ComputedDefaultValue`: the client may supply a value, and the server generates one where it
   * doesn't. Hence the property is editable, but never required.
   */
  optionalWithDefault = "optionalWithDefault",
  /**
   * `Core.Immutable`: the client may supply a value on create, but not change it afterwards.
   */
...
export enum ManagedPropertyMode {
   ...
   * dropped entirely - wired into the generated service's `patch()` method.
   */
...
export interface CliOptions {
   ...
   * editable. By default, the annotations of the service are evaluated and the well known rule "the only
   * key prop of an entity is generated by the server" applies wherever they say nothing.
   *
...
export interface PropertyGenerationOptions extends RenameOptions {
  /**
   * In which way this property is managed by the server, see {@link ManagedState}. Configuring it here
   * always wins over what the annotations of the service say and over the key detection.
   *
   * The booleans are shorthands: `true` for {@link ManagedState.readOnly} - a managed attribute cannot
   * be created or updated and is hence left out of the editable model versions - and `false` for
   * {@link ManagedState.off}.
   */
  managed?: boolean | ManagedState;
