...
export enum ManagedPropertyDetection {
  /**
   * Evaluate the annotations of the service, and fall back to the {@link simpleHeuristic} where they say
   * nothing. The default, and the best of both worlds: authoritative wherever the service is explicit,
   * still convenient for the many services which state no annotations at all.
   */
  auto = "auto",
  /**
  ...
  /**
   * Ignore the annotations of the service and go by the rule that a single key prop - as opposed to one
   * part of a composite key - is generated by the server. This heuristic only ever concerns keys, so no
   * other property is managed under this setting.
   */
  simpleHeuristic = "simpleHeuristic",
  /**
   * Derive nothing at all: a property is managed only where the configuration says so.
   */
  none = "none",
}
