...
export class BookService<in out ClientType extends ODataHttpClient> extends EntityTypeServiceV4<
  ...
  QBook
> {
  private _altChoices?: CollectionServiceV4<ClientType, EnumCollection<Choice>, QEnumCollection<Choice>>;
  ...
  public altChoices() {
    if (!this._altChoices) {
      const { client, path } = this.__base;
      this._altChoices = new CollectionServiceV4(client, path, "altChoices", new QEnumCollection(Choice));
    }
