...
> {
  ...
  public getLectorSrv(): ReviewerService<ClientType> {
    if (!this._lectorSrv) {
      this._lectorSrv = new ReviewerService(this.client, this.getPath(), "lector");
    }
  ...
  public getReviewersSrv(): CollectionServiceV2<ClientType, Reviewer, QReviewer, EditableReviewer> {
    if (!this._reviewersSrv) {
      this._reviewersSrv = new CollectionServiceV2(this.client, this.getPath(), "reviewers", qReviewer);
    }
