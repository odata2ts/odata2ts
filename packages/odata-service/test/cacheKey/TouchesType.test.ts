...
describe("touchesType - array needle: an invalidates entry, wherever it occurs", () => {
  ...
  test("a specific-key needle reaches the same entity addressed as a hop under an unrelated parent", () => {
    ...
    expect(touchesType([COPY, "detail", { MediumId: 5, InventoryNumber: 7 }], key)).toBe(true);
  });
  ...
  test("a specific-key needle does not match a different key reached the same way", () => {
    ...
    expect(touchesType([COPY, "detail", { MediumId: 9, InventoryNumber: 1 }], key)).toBe(false);
  });
  ...
  test("the skip never applies anywhere but immediately after a kind marker", () => {
    ...
    expect(touchesType([MEDIUM, "detail", 5], key)).toBe(false);
  });
  ...
  test("an empty key matches nothing", () => {
    expect(touchesType([MEDIUM, "detail", 5], [])).toBe(false);
  });
});
