...
export type KeyObjectExtractor<EIdType> = Extract<EIdType, object>;
// Filter out id field when creating a new entity, at least filter out Guids, since they are generated by definition
export type Creatable<EModel, EIdType> = KeyObjectExtractor<EIdType> extends { [key: string]: GuidString }
  ? Omit<Unnominalized<EModel>, keyof KeyObjectExtractor<EIdType>>
  : Unnominalized<EModel>;
...
export abstract class EntitySetService<EModel, EIdType> extends EntityBaseService<EModel> {
  ...
  public create(model: Creatable<EModel, EIdType>): ODataResponse<ODataModelResponse<Unnominalized<EModel>>> {
