...
describe("QBigNumberPath test", () => {
  let toTest: QBigNumberPath;
  let otherProp: QBigNumberPath;

  beforeEach(() => {
    toTest = new QBigNumberPath("Price");
    otherProp = new QBigNumberPath("Budget");
  });
  ...
  test("fails with null, undefined, empty string", () => {
    // @ts-expect-error
    expect(() => new QBigNumberPath(null)).toThrow();
    // @ts-expect-error
    expect(() => new QBigNumberPath()).toThrow();
    // @ts-expect-error
    expect(() => new QBigNumberPath(undefined)).toThrow();
    expect(() => new QBigNumberPath("")).toThrow();
    expect(() => new QBigNumberPath(" ")).toThrow();
  });

  test("with converter", () => {
    const testWithConv = new QBigNumberPath("ID", stringToPrefixModelConverter);

    expect(testWithConv.gt({ prefix: "pref", value: "12" }).toString()).toBe(`ID gt 12`);
  });
  ...
  test("equals", () => {
    ...
    expect(result.toString()).toBe("Price eq 4.2");
